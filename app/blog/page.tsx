...
export default function BlogPage() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section ref={heroRef} className="relative min-h-[65vh] md:min-h-[70vh] bg-background text-foreground flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-12 lg:px-20 py-24 md:py-32 pt-32 md:pt-40">
          <div className="max-w-3xl">
            <div className="blog-hero-reveal inline-flex items-center p-1 px-3 md:px-4 mb-6 md:mb-8 bg-foreground/5 rounded-full border border-foreground/10">
              <span className="w-2 h-2 bg-lime rounded-full mr-2 md:mr-3 animate-pulse" />
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-foreground/60">Blog & Insights</span>
            </div>
            
            <h1 className="blog-hero-reveal text-[2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[5rem] xl:text-[6rem] font-[900] leading-[0.95] tracking-tighter uppercase mb-6 md:mb-8 text-foreground">
              Ideas, Insights
              <br />
              <span className="text-lime italic">& Innovation</span>
            </h1>
            
            <p className="blog-hero-reveal text-foreground/50 text-base md:text-lg lg:text-xl leading-relaxed mb-8 md:mb-10 max-w-2xl">
            ...
            <div className="blog-hero-reveal relative max-w-xl">
              <Search className="absolute left-4 md:left-5 top-1/2 -translate-y-1/2 w-4 md:w-5 h-4 md:h-5 text-foreground/40" />
              <input
                type="text"
                placeholder="Search articles..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-foreground/5 border border-foreground/10 rounded-full py-3 md:py-4 pl-11 md:pl-14 pr-4 md:pr-6 text-sm md:text-base text-foreground placeholder:text-foreground/40 focus:outline-none focus:border-lime/50 transition-colors"
              />
            </div>
          </div>
        </div>
      ...
      <section className="py-12 md:py-20 lg:py-24 px-4 sm:px-6 md:px-12 lg:px-20 bg-background text-foreground">
        <div className="max-w-7xl mx-auto">
          <ScrollReveal className="mb-8 md:mb-12">
            <div className="inline-flex items-center p-1 px-3 md:px-4 mb-3 md:mb-4 bg-foreground/5 rounded-full">
              <TrendingUp className="w-3 h-3 mr-2" />
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-foreground/60">Featured Articles</span>
            </div>
          </ScrollReveal>
      ...
      <section className="py-12 md:py-20 lg:py-24 px-4 sm:px-6 md:px-12 lg:px-20 bg-foreground/[0.03] border-t border-foreground/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-3 gap-8 md:gap-12">
            ...
            <div className="lg:col-span-2">
                      ...
                      <div className="grid sm:grid-cols-3 gap-0">
                        ...
                        <div className="sm:col-span-2 p-4 md:p-6">
                          <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                            <span className="px-2 py-0.5 md:py-1 bg-lime/10 text-lime rounded text-[10px] md:text-xs font-bold">
                              {post.category}
                            </span>
                            <span className="text-foreground/40 text-[10px] md:text-xs flex items-center gap-1">
                              <Calendar className="w-3 h-3" />
                              {post.date}
                            </span>
                          </div>
                          <h3 className="text-base md:text-lg font-bold mb-1 md:mb-2 group-hover:text-lime transition-colors text-foreground leading-snug">
                            {post.title}
                          </h3>
                          <p className="text-foreground/40 text-xs md:text-sm line-clamp-2 mb-3 md:mb-4">
                            {post.excerpt}
                          </p>
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-foreground/30 text-[10px] md:text-xs">{post.readTime}</span>
                          </div>
                        </div>
                      </div>
                    </Link>
              ...
              {filteredPosts.filter(p => !p.featured).length === 0 && (
                <div className="text-center py-12 md:py-20">
                  <p className="text-foreground/40">No articles found matching your criteria.</p>
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-1 space-y-6 md:space-y-8">
              {/* Categories */}
              <ScrollReveal>
                <div className="bg-foreground/5 rounded-xl md:rounded-2xl p-4 md:p-6 border border-foreground/10">
                  <h3 className="text-base md:text-lg font-black uppercase tracking-tight mb-3 md:mb-4 text-foreground">Categories</h3>
              ...
              <ScrollReveal delay={0.1}>
                <div className="bg-foreground/5 rounded-xl md:rounded-2xl p-4 md:p-6 border border-foreground/10">
                  <h3 className="text-base md:text-lg font-black uppercase tracking-tight mb-3 md:mb-4 text-foreground">Trending Topics</h3>
                  <div className="flex flex-wrap gap-2">
                    {trendingTopics.map((topic, index) => (
                      <span 
                        key={index}
                        className="px-2 md:px-3 py-1.5 md:py-2 bg-foreground/5 rounded-full text-[10px] md:text-xs font-bold text-foreground/60 hover:bg-lime hover:text-black transition-colors cursor-pointer"
                      >
                        {topic}
                      </span>
                    ))}
