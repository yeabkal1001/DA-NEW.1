...
export default function ProjectsPage() {
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* Hero Section */}
      <section ref={heroRef} className="relative min-h-[75vh] md:min-h-[80vh] bg-black text-white flex items-center overflow-hidden">
        {/* Background */}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-12 lg:px-20 py-24 md:py-32 pt-32 md:pt-40">
          <div className="max-w-4xl">
            <div className="projects-hero-reveal inline-flex items-center p-1 px-3 md:px-4 mb-6 md:mb-8 bg-white/5 rounded-full border border-white/10">
            ...
            <h1 className="projects-hero-reveal text-[2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[5rem] xl:text-[6rem] font-[900] leading-[0.95] tracking-tighter uppercase mb-6 md:mb-8">
              Showcasing
              <br />
              <span className="text-lime italic">Digital Excellence</span>
            </h1>
            
            <p className="projects-hero-reveal text-white/50 text-base md:text-lg lg:text-xl leading-relaxed mb-8 md:mb-10 max-w-2xl">
              Explore our portfolio of successful projects spanning web development, mobile apps, and enterprise solutions.
            </p>

            {/* Stats Row */}
            <div className="projects-hero-reveal grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6 pt-8 md:pt-10 border-t border-white/10">
              {stats.map((stat, index) => (
                <div key={index}>
                  <div className="text-xl sm:text-2xl md:text-2xl lg:text-4xl font-black text-lime">{stat.number}</div>
                  <div className="text-[10px] md:text-xs lg:text-sm text-white/40 font-medium uppercase tracking-wider">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
                ...
                <Link href={`/projects/${project.id}`} className="group block bg-white rounded-xl md:rounded-2xl overflow-hidden border border-black/5 hover:border-lime/50 hover:shadow-xl transition-all duration-300">
                  <div className="relative h-40 md:h-56 overflow-hidden">
                    ...
                    <div className="absolute top-3 md:top-4 right-3 md:right-4">
                      <span className="px-2 md:px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-[10px] md:text-xs font-bold">
                        {project.year}
                      </span>
                    </div>
                  </div>
                  <div className="p-4 md:p-6">
                    ...
                    <h3 className="text-base md:text-xl font-bold mb-1 md:mb-2 group-hover:text-lime transition-colors">{project.title}</h3>
                    <p className="text-gray-500 text-xs md:text-sm line-clamp-2 mb-3 md:mb-4">{project.description}</p>
        ...
        <div className="max-w-5xl mx-auto text-center">
          <ScrollReveal>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
              <Link 
                href="/contact"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 md:gap-3 bg-lime text-black px-8 md:px-10 py-4 md:py-5 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-white transition-all duration-500 group"
              >
                Start Your Project
              ...
              <Link 
                href="/services"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 md:gap-3 border border-white/20 text-white px-8 md:px-10 py-4 md:py-5 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-white/10 transition-all duration-500"
              >
                View Services
              </Link>
            </div>
