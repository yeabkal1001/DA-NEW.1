...
export default function ProjectsPage() {
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section ref={heroRef} className="relative min-h-[75vh] md:min-h-[80vh] bg-background text-foreground flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-12 lg:px-20 py-24 md:py-32 pt-32 md:pt-40">
          <div className="max-w-4xl">
            <div className="projects-hero-reveal inline-flex items-center p-1 px-3 md:px-4 mb-6 md:mb-8 bg-foreground/5 rounded-full border border-foreground/10">
              <span className="w-2 h-2 bg-lime rounded-full mr-2 md:mr-3 animate-pulse" />
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-foreground/60">Our Portfolio</span>
            </div>
            
            <h1 className="projects-hero-reveal text-[2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[5rem] xl:text-[6rem] font-[900] leading-[0.95] tracking-tighter uppercase mb-6 md:mb-8 text-foreground">
              Showcasing
              <br />
              <span className="text-lime italic">Digital Excellence</span>
            </h1>
            
            <p className="projects-hero-reveal text-foreground/50 text-base md:text-lg lg:text-xl leading-relaxed mb-8 md:mb-10 max-w-2xl">
              Explore our portfolio of successful projects spanning web development, mobile apps, and enterprise solutions.
            </p>

            {/* Stats Row */}
            <div className="projects-hero-reveal grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6 pt-8 md:pt-10 border-t border-foreground/10">
              {stats.map((stat, index) => (
                <div key={index}>
                  <div className="text-xl sm:text-2xl md:text-2xl lg:text-4xl font-black text-lime">{stat.number}</div>
                  <div className="text-[10px] md:text-xs lg:text-sm text-foreground/40 font-medium uppercase tracking-wider">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <ScrollReveal className="mb-10 md:mb-16">
            <div className="inline-flex items-center p-1 px-3 md:px-4 mb-4 md:mb-6 bg-foreground/5 rounded-full border border-foreground/10">
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-foreground/60">Featured Work</span>
            </div>
            <h2 className="text-[1.75rem] sm:text-[2rem] md:text-[3rem] font-[900] leading-[0.95] tracking-tighter uppercase text-foreground">
          ...
          <ScrollReveal delay={0.3} className="mt-4 md:mt-6">
            <div className="group relative bg-lime rounded-[1.5rem] md:rounded-[2rem] overflow-hidden border border-foreground/10 shadow-sm transition-colors duration-500">
              <div className="grid lg:grid-cols-2 gap-0">
                <div className="p-6 md:p-10 lg:p-14 flex flex-col justify-center">
                  <div className="flex flex-wrap gap-2 mb-3 md:mb-4">
                    {featuredProjects[2]?.tags.map((tag, i) => (
                      <span key={i} className="px-2 md:px-3 py-1 bg-black/10 rounded-full text-[10px] md:text-xs font-bold text-black/80">
                        {tag}
                      </span>
                    ))}
                  </div>
                  <h3 className="text-2xl md:text-3xl lg:text-4xl font-black uppercase tracking-tight mb-3 md:mb-4 text-black">
                  ...
                  <button className="inline-flex items-center gap-2 text-xs md:text-sm font-black uppercase tracking-wider group/btn text-black">
                    View Case Study
        ...
        <div className="max-w-7xl mx-auto">
          <ScrollReveal className="mb-8 md:mb-12">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-6">
              <div>
                <h2 className="text-[1.75rem] sm:text-[2rem] md:text-[3rem] font-[900] leading-[0.95] tracking-tighter uppercase mb-2 md:mb-4 text-foreground">
                  All <span className="text-lime italic">Projects</span>
                </h2>
                <p className="text-foreground/40 text-sm md:text-base">Browse our complete portfolio</p>
              </div>
              ...
              <ScrollReveal key={project.id} delay={index * 0.05}>
                <Link href={`/projects/${project.id}`} className="group block bg-foreground/5 rounded-xl md:rounded-2xl overflow-hidden border border-foreground/5 hover:border-lime/50 transition-all duration-300">
                  ...
                  <div className="p-4 md:p-6">
                    ...
                    <h3 className="text-base md:text-xl font-bold mb-1 md:mb-2 group-hover:text-lime transition-colors text-foreground">{project.title}</h3>
                    <p className="text-foreground/50 text-xs md:text-sm line-clamp-2 mb-3 md:mb-4">{project.description}</p>
                    <div className="flex flex-wrap gap-1 md:gap-2">
                      {project.tags.slice(0, 2).map((tag, i) => (
                        <span key={i} className="px-2 py-0.5 md:py-1 bg-foreground/10 rounded text-[10px] md:text-xs font-medium">
                          {tag}
                        </span>
                      ))}
                    </div>
      ...
      {/* CTA Section */}
      <section className="py-16 md:py-24 lg:py-32 px-4 sm:px-6 md:px-12 lg:px-20 bg-background border-t border-foreground/5 text-foreground">
        <div className="max-w-5xl mx-auto text-center">
          <ScrollReveal>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
              <Link 
                href="/contact"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 md:gap-3 bg-lime text-black px-8 md:px-10 py-4 md:py-5 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-foreground hover:text-background transition-all duration-500 group"
              >
                Start Your Project
              ...
              <Link 
                href="/services"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 md:gap-3 border border-foreground/20 text-foreground px-8 md:px-10 py-4 md:py-5 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-foreground/10 transition-all duration-500"
              >
                View Services
              </Link>
            </div>
